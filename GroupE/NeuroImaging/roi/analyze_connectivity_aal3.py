...
df = df[df["filepath"].str.contains("aal3", case=False)]
...
for group in ["control", "patient"]:
    if avg_corr[group] is not None:
        matrix_path = os.path.join(output_dir, f"connectivity_matrix_{group}_aal3.npy")
...
for group in ["control", "patient"]:
    if avg_corr[group] is not None:
        ...
        plt.title(f"Average Correlation Matrix ({group.title()} Group) - AAL3")
        heatmap_path = os.path.join(plot_dir, f"correlation_heatmap_{group}_aal3.png")
